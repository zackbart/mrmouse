...
public enum ReprogControls {
    ...
    public static func setControlReporting(device: HIDPPDevice, controlID: UInt16, reporting: ControlReporting) throws {
        ...
        let idHi = UInt8((controlID >> 8) & 0xFF)
        let idLo = UInt8(controlID & 0xFF)
        // V4 SetControlReporting: [controlID_hi, controlID_lo, flags, remapped_hi, remapped_lo]
        // remapped = the CID this button maps to when not diverted (usually the same CID).
        let params = Data([idHi, idLo, reporting.flags.rawValue, idHi, idLo])
        NSLog("[ReprogControls] setControlReporting: idx=0x%02X params=%@",
              idx, params.map { String(format: "%02X", $0) }.joined(separator: " "))
