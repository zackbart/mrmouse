...
public enum DeviceInfo {
    ...
    public static func getDeviceName(device: HIDPPDevice) throws -> String {
        ...
        // 2. Fetch name in chunks (up to 16 bytes per call — long report has 16 params).
        var nameBytes = Data()
        let chunkSize = 16
        var offset = 0
        ...
        while nameBytes.count < totalLength {
            ...
            // HID++ 2.0 GetName returns name bytes directly starting at the requested offset
            // (no echoed offset byte in the response).
            guard response.parameters.count >= 1 else {
                throw HIDPPDeviceError.unexpectedResponse
            }
            let chunk = response.parameters
