        ...
        VStack(alignment: .leading, spacing: 12) {
            VStack(alignment: .leading, spacing: 6) {
                Toggle("Smart Shift", isOn: $smartShiftEnabled)
                    .font(.subheadline)
                    .onChange(of: smartShiftEnabled) { commit() }
                ...
                if smartShiftEnabled {
                    HStack {
                        ...
                        Slider(value: $smartShiftThreshold, in: 1...255, step: 1)
                            .onChange(of: smartShiftThreshold) { commit() }
            ...
            Toggle("High-Resolution Scrolling", isOn: $hiResScrollEnabled)
                .font(.subheadline)
                .onChange(of: hiResScrollEnabled) { commit() }

            Divider()

            Toggle("Reverse Scroll Direction", isOn: $scrollInverted)
                .font(.subheadline)
                .onChange(of: scrollInverted) { commit() }

            Toggle("Reverse Thumb Wheel", isOn: $thumbWheelInverted)
                .font(.subheadline)
                .onChange(of: thumbWheelInverted) { commit() }
        }
        .onAppear { loadFromConfig() }
        .onChange(of: appState.selectedDeviceID) { loadFromConfig() }
    }
